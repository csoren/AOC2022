...
let first_puzzle () =
  let rounds = List.map round_of_string_pair input in
...
let second_puzzle () =
  let outcomes = List.map hand_winner_of_string_pair input in
  let rounds = List.map (fun (your, winner) -> (your, determine_winning_hand winner your)) outcomes in
